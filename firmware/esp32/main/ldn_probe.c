...
#error "This independent firmware requires classic ESP32"
#endif
...
static unsigned s_connect_attempts;
static bool s_joining;
static atomic_int s_disconnect_reason = -1;
static atomic_bool s_reset_pending;

static void log_wifi_state(uint8_t reason)
{
    printf("LDN_STATE reason=%u joining=%u ssid0=%u target=" MACSTR
           " pending=%u attempts=%u assoc=%u\n",
           reason, s_joining, s_ssid[0] != 0, MAC2STR(s_target_bssid),
           atomic_load(&s_reset_pending), s_connect_attempts, s_association_seen);
}
...
static bool install_ldn_keys(void)
{
    ...
    /* Match the verified Classic ESP32 baseline: populate CAM first, then open
       the controlled port. Reinstalling around auth_done can leave the
       driver's WPA/CAM state inconsistent and provoke reason 242. */
    ...
    if (pairwise != 0 || group != 0) {
        ESP_LOGE(TAG, "CCMP preload failed: pairwise=%d group=%d",
                 pairwise, group);
        return false;
    }

    /* Readback is diagnostic only; encrypted LDN traffic is the real check. */
    const bool pairwise_ok = read_key_back(0, LDN_KEY_FLAG_PAIRWISE, s_ccmp_key);
    ...
    printf("LDN_KEY_STATUS pairwise_result=%d group_result=%d pairwise_readback=%u group_readback=%u\n",
           pairwise, group, pairwise_ok, group_ok);
                       ...
                       void *data)
{
    ...
    if (id == WIFI_EVENT_STA_CONNECTED) {
        ...
        /* Match S3/C6: register the LDN RX callback as soon as the station is
           associated.  The host still waits 50 ms before its first TX, while
           the owner task installs the derived CCMP keys before that window. */
        ldn_control_link(true);
    } else if (id == WIFI_EVENT_STA_DISCONNECTED) {
        const wifi_event_sta_disconnected_t *event = data;
        ESP_LOGW(TAG, "STA_DISCONNECTED reason=%u", event->reason);
        log_wifi_state(event->reason);
        atomic_store(&s_disconnect_reason, event->reason);
        ldn_control_link(false);
        if (s_joining && s_connect_attempts < 8)
            s_reconnect_at = esp_timer_get_time() + 500000;
        else if (s_ssid[0])
            /* Defer stop/start to the owner task; Wi-Fi APIs from this event
               callback can deadlock the driver. */
            atomic_store(&s_reset_pending, true);
    }
}
...
void ldn_session_stop(void)
{
    ...
    /* Match S3/C6: EAPOL is consumed by the WPA hook, so promiscuous mode
       only needs management frames and should not compete for RX buffers. */
...
esp_err_t ldn_session_configure(const char *ssid, const char *bssid, const char *key, unsigned channel)
{
    ...
    /* The LDN action-frame source is the AP BSSID.  Lock association to that
       BSSID like the S3/C6 implementations; accepting any same-SSID AP can
       turn a captured-but-stale advertisement into reason 201 (NO_AP_FOUND). */
    ...
    s_association_seen = false;
    s_reconnect_at = 0;
    s_connect_attempts = 1;
    s_joining = true; s_join_started = esp_timer_get_time();
...
static void run_private_join(void)
{
    ...
    for (;;) {
        ...
        const bool reset_pending = atomic_exchange(&s_reset_pending, false);
        /* Reason 242 is emitted by the LDN peer after an authenticated
           station is kicked. It is not a normal association retry; tear down
           the old key/socket context before accepting another LDN_CONFIG. */
        if (reason == 242) {
            printf("LDN_SESSION_RESET %d\n", reason);
            ldn_session_stop();
        } else if (reset_pending && !s_joining && s_ssid[0]) {
            printf("LDN_SESSION_RESET %d\n", reason);
            ldn_session_stop();
        }
        const int64_t now = esp_timer_get_time();
        if (s_joining && s_reconnect_at && now >= s_reconnect_at) {
            s_reconnect_at = 0;
            const esp_err_t retry = esp_wifi_connect();
            if (retry == ESP_OK) {
                ++s_connect_attempts;
                printf("LDN_RECONNECT %u\n", s_connect_attempts);
            } else {
                printf("LDN_RECONNECT_ERROR %d\n", retry);
            }
        }
        if (s_joining && s_association_seen && esp_wifi_sta_is_running_internal()) {
            s_joining = false;
            if (install_ldn_keys()) ldn_control_link(true);
        } else if (s_joining && now - s_join_started > 30000000) {
