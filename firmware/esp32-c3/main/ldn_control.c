...
void ldn_control_init(esp_netif_t *netif, const unsigned char host[6])
{
    ...
    printf("LDN_C3_READY transport=usb-serial-jtag heap=%" PRIu32 "\n", esp_get_free_heap_size());
}
...
static void command(const char *line)
{
    ...
    if (!strcmp(line, "LDN_STATUS")) {
        ...
        printf("LDN_C3_STATS tx_ok=%u tx_failed=%u ethernet_rx=%u air_rx=%u dropped=%u usb_dropped=%" PRIu32 " heap=%" PRIu32 "\n",
            atomic_load(&s_tx_ok), atomic_load(&s_tx_failed), atomic_load(&s_rx_count), atomic_load(&s_air_data),
            atomic_load(&s_dropped), c3_transport_dropped(), esp_get_free_heap_size());
        return;
    }
...
void ldn_control_poll(void)
{
    ...
    int n = c3_transport_read(bytes, sizeof(bytes));
    for (int i = 0; i < n; ++i) {
