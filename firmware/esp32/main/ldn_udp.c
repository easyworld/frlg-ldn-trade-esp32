...
static unsigned s_tx, s_rx, s_rejected, s_retries, s_drops;
...
static int start(const char *ours, const char *host)
{
    ...
    if (setsockopt(s_socket, SOL_SOCKET, SO_BROADCAST, &yes, sizeof(yes)) ||
        bind(s_socket, (struct sockaddr *)&address, sizeof(address))) goto failed;
    s_tx = s_rx = s_rejected = s_retries = s_drops = 0;
...
bool ldn_udp_command(const char *line, bool connected)
{
    if (!strcmp(line, "LDN_PING")) {
        s_heartbeat = esp_timer_get_time();
        printf("LDN_PONG %u %u %u retries=%u drops=%u\n",
               s_tx, s_rx, s_rejected, s_retries, s_drops);
        ...
        /* A blocking socket can still report ENOMEM while lwIP's TX pbuf pool
           is briefly saturated.  PIA already handles UDP loss, but first give
           the driver a bounded opportunity to drain without stalling UART. */
        int sent = -1;
        int send_errno = 0;
        for (int attempt = 0; attempt < 8; ++attempt) {
            sent = sendto(s_socket, payload, n / 2, 0,
                          (struct sockaddr *)&dest, sizeof(dest));
            if (sent == (int)(n / 2)) break;
            send_errno = errno;
            if (send_errno != ENOMEM && send_errno != ENOBUFS &&
                send_errno != EAGAIN && send_errno != EWOULDBLOCK) break;
            if (attempt + 1 < 8) {
                ++s_retries;
                vTaskDelay(pdMS_TO_TICKS(2));
            }
        }
        if (sent == (int)(n / 2)) result = ESP_OK;
        else if (send_errno == ENOMEM || send_errno == ENOBUFS ||
            ...
            result = ESP_FAIL;
            printf("LDN_UDP_ERROR %d errno=%d\n", result, send_errno);
        }
tx_done:
        if (result == ESP_OK) ++s_tx;
        else if (result != ESP_FAIL || errno == 0) { ++s_rejected; printf("LDN_UDP_ERROR %d\n", result); }
    } else return false;
    return true;
}
...
void ldn_udp_poll(bool connected)
{
    ...
    for (int count = 0; count < 2; ++count) {
        ...
        int n = recvfrom(s_socket, bytes, sizeof(bytes), MSG_DONTWAIT,
